...
async def generate(
    ...
    forge_settings = {
        ...
        "scheduler": llm_result.get("scheduler", "Karras"),
        "width": llm_result.get("width", 1024),
        "height": llm_result.get("height", 1024),
    }

    image_path, forge_error = await generate_image_in_forge(
        prompt, negative_prompt, forge_settings
    )
    if not image_path:
        if isinstance(forge_error, dict) and forge_error.get("error"):
            raise HTTPException(
                status_code=forge_error.get("status_code", 500),
                detail=forge_error["error"],
            )
        raise HTTPException(status_code=500, detail=forge_error or "Forge failed")

    await crud.create_history_record(
        db, current_user.id, request.text, prompt, image_path
    )
